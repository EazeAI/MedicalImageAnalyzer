...
batch = 1
epoch = 20
# testing_folder = "uploadr/static/uploads/3b11e8d3-438e-4bf9-9791-3d7f772dbe1d"
...
def runProgram(target):
    print("hello")
    testing_folder = target
    testing_folder_len = len([name for name in os.listdir(os.getcwd()+"/"+testing_folder)])
    epsilon = 1e-3
    g2 = tf.Graph()
    with g2.as_default():
    ...
    with tf.Session(graph=g2) as sess1:
        ...
        for j in range(0,r,25):
            ...
            normal = str(round(s[0][0],3))
            nodule = str(round(s[0][1],3))
            truedisease = ind[str(actual[0])]
            # print type(pred)
    ...
    print (conf_matrix)
    # cmatrix = pd.Series(conf_matrix).to_json(orient='values')
    ...
    data = {}
    data['conf_matrix'] = str(conf_matrix)
    data['precision'] = str(prfs[0])
    data['recall'] = str(prfs[1])
    data['fscore'] = str(prfs[2])
    data['support'] = str(prfs[3])
    data['class_pred'] = str(class_pred)
    data['class_actual'] = str(class_actual)
    data['pred'] = str(pred)
